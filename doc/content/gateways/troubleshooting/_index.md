...
- Many downlinks being scheduled at the same time via single gateway - this usually happens if a region is poorly equipped with gateways. The solution would be to deploy more gateways in the region, i.e. improve the network coverage.
- Full duty cycles on the gateway - this can happen if the gateway is handling a lot of traffic, or the end devices are using SF10 or higher. Try adding more gateways to improve the network coverage in your region.
...
Please try scheduling downlinks with the frequency channels supported by the gateway and the Network Server.
